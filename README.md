...
working, and a small password store where you can save a username with its password and look that
password up again later. Everything runs locally and is kept in the computer's memory, so nothing is
...
|-----------|--------|----------|
| Username was never added | `404` | `{"error": "Username 'unknown' not found"}` |
...
| Wrong method used on an endpoint | `405` | `{"error": "Method not allowed. Check whether this endpoint expects GET or POST."}` |
